...
export class LobsterDocument implements TextDocument {
	...
	public async getRelative(lsp: LSPInstance): Promise<string> {
		...
		const folder = (folders && folders.length>0) ? folders.filter(
			(folder) => this._uri.fsPath.startsWith(URI.parse(folder.uri).fsPath)
		)[0].uri : '';
*/
        const folder = '';
        lsp.connection.console.log("getRelative folder = "+folder);
	...
	public async writeToTmp(lsp: LSPInstance): Promise<string> {
        lsp.connection.console.log("writeToTmp start");
		if (lsp.errored()) throw "LSP has errored";

        lsp.connection.console.log("writeToTmp getRelative ");
		const relative = await this.getRelative(lsp);
        lsp.connection.console.log("writeToTmp path.join");
		const join = path.join(lsp.tempDir, relative);
        lsp.connection.console.log("writeToTmp path.parse");
		const reldir = path.parse(relative).dir;
        lsp.connection.console.log("writeToTmp relative = "+relative);
        lsp.connection.console.log("writeToTmp join = "+join);
        lsp.connection.console.log("writeToTmp reldir = "+reldir);
		...
		if (reldir.length != 0) {
			await new Promise(
				(resolve, fail) => fs.mkdir(path.join(lsp.tempDir, reldir), { recursive: true },
					(err, path) => {
						if (err) {
                            lsp.connection.console.log("writeToTmp failed write to create directory "+path+": "+err);
							console.error(err);
							fail(err);
						} else {
							resolve(path);
        ...
        lsp.connection.console.log("writeToTmp filepath: "+join);
		await new Promise((resolve, fail) => {
            if (fail) lsp.connection.console.log("writeToTmp failed to open file: "+fail);
			fs.open(join, 'w', (err, fd) => {
				if (err) {
                    lsp.connection.console.log("writeToTmp failed to open file: "+err);
					fail(err);
					return;
				} else {
					fs.writeFile(fd, this._content, (err) => {
						if (err) {
                            lsp.connection.console.log("writeToTmp failed to write file: "+err);
							...
							fs.close(fd, (err) => {
								if (err) {
                                    lsp.connection.console.log("writeToTmp failed to close file: "+err);
									fail(err);
									return;
								} else {
									resolve(null);
	...
	public async parse(lsp: LSPInstance): Promise<Diagnostic[]> {
        lsp.connection.console.log("parse start");
		const settings = await lsp.getDocumentSettings(this._uri);
        lsp.connection.console.log("parse got settings");
		if (lsp.errored()) {
            this._state = LobsterDocumentState.HasErrors;
            return [{range: { start: { line: 5, character: 23 }, 
        ...
        lsp.connection.console.log("parse parsed");

		this._state = LobsterDocumentState.NoErrors;
		result.forEach((diag) => {
            lsp.connection.console.log("parse diag");
...
export namespace LobsterDocument {
	...
	export function update(
        ...
        console.log("Got TextDocumentContentChangeEvent[]");
